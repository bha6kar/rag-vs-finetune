...
Everything here is reproducible. The full setup:

- **Documents and questions.** Four real SEC filings (Apple FY2024, Berkshire 2024, AMD and Boeing FY2022). 54 questions; 17 held out for the test, stratified so every document appears. The fine-tune never sees the held-out 17.
- **Judge.** A single LLM judge (`gpt-5.4-mini`) scores every generated answer on two binary criteria, correct and faithful, using one fixed prompt. The RAG condition's per-question verdicts come from the part-one bake-off, which used the same judge.
- **Stack.** Hosted models (the judge, `gpt-5.4-mini`) run on Azure OpenAI; Claude Haiku 4.5 runs via OpenRouter; the open model is trained and run locally with MLX on an Apple Silicon Mac (a 48 GB M4 Pro). No CUDA.
- **Knowledge fine-tune.** A LoRA adapter on Qwen2.5-7B-Instruct, trained closed-book on 513 question/answer pairs generated by the judge model from the same document chunks RAG retrieves from, so fine-tuning and RAG draw on identical source material. 400 iterations, learning rate 1e-5; final training loss settled around 0.28 (learning, not the near-zero of rote memorisation).
- **Format fine-tune.** A second LoRA adapter on the same base, trained on 228 (question + context, schema JSON) pairs, then evaluated against the prompt-only model with retrieved context. Adherence is scored strictly: the raw output must parse as a bare JSON object with exactly the required keys and a valid confidence value.
- **What is measured vs general.** Every number in the two appendices is measured on these documents. The step-by-step workflow earlier in this guide is informed by the literature and by these results, not a separate benchmark.

Scripts, the train/test split, the generated corpora's build code, and the full per-question results are in the [rag-vs-finetune repository](https://github.com/bha6kar/rag-vs-finetune); the part-one bake-off is at [rag-bake-off](https://github.com/bha6kar/rag-bake-off).
