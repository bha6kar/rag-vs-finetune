...
What this experiment did *not* test is fine-tuning for behaviour and format, the thing fine-tuning is actually good at. I would expect a fine-tune to win there, and I say so plainly below. But for getting facts out of your documents, the measured gap is not subtle: the same open model goes from 65% correct with retrieval to a hallucinating 24% when you try to bake the facts into its weights, even with a corpus built for the job.
...
- The fine-tuning numbers I *did* run test one thing: knowledge injection, closed-book. I gave it a fair, well-resourced attempt (513 Q/A drawn from the documents, trained to learn not memorise) and it still lost to RAG by a wide margin. This is not a test of fine-tuning for behaviour and format, which I did not benchmark and which I expect a fine-tune to win. Do not read "fine-tune scored 24%" as "fine-tuning is useless"; read it as "do not fine-tune facts you could retrieve".
